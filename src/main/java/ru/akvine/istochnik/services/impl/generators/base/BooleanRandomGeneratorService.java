...
public class BooleanRandomGeneratorService extends AbstractBaseTypeGeneratorService {
    ...
    @Autowired
    protected BooleanRandomGeneratorService(
    ...
    public List<?> generate(Config config, List<Converter> converters) {
        ConfigMapperService<? extends ru.akvine.istochnik.services.generators.Config> configMapper =
                configMappersProvider.configMappers().get(getType().getValue());
        BooleanGeneratorConfig mappedConfig = (BooleanGeneratorConfig) configMapper.map(config);
        return apply(booleanGeneratorService.generate(mappedConfig), converters, config.getRandomGenerator());
    }

...
public class ProvidersDelegateConfig {
    ...
    @Bean
    public DateTimeRangeServicesProvider dateTimeShiftServicesProvider(
            Collection<AbstractDateTimeRangeService<LocalDateTime, Long>> services) {
        Map<DateShiftType, AbstractDateTimeRangeService<LocalDateTime, Long>> localDateTimeShiftServices =
                services.stream().collect(toMap(AbstractDateTimeRangeService::getByType, identity()));
        return new DateTimeRangeServicesProvider(localDateTimeShiftServices);
    }

    @Bean
    public TimeRangeServicesProvider timeRangeServicesProvider(
            Collection<AbstractTimeRangeService<LocalTime, Integer>> services) {
        Map<DateShiftType, AbstractTimeRangeService<LocalTime, Integer>> timeShiftServices =
                services.stream().collect(toMap(AbstractTimeRangeService::getByType, identity()));
        return new TimeRangeServicesProvider(timeShiftServices);
    }

    @Bean
    public DateRangeServicesProvider dateRangeServicesProvider(
            Collection<AbstractDateRangeService<LocalDate, Integer>> services) {
        Map<DateShiftType, AbstractDateRangeService<LocalDate, Integer>> localDateShiftServices =
                services.stream().collect(toMap(AbstractDateRangeService::getByType, identity()));
    ...
    public IntegerConvertersProvider integerConvertersProvider(List<IntegerConverter<Long, Double>> integerConverters) {
        Map<ConverterType, IntegerConverter<Long, Double>> converters =
                integerConverters.stream().collect(toMap(IntegerConverter::getName, identity()));
    ...
    public DoubleConvertersProvider doubleConvertersProvider(List<DoubleConverter<Double, Double>> doubleConverters) {
        Map<ConverterType, DoubleConverter<Double, Double>> converters =
                doubleConverters.stream().collect(toMap(DoubleConverter::getName, identity()));
    ...
    public StringConvertersProvider stringConvertersProvider(List<StringConverter<String, String>> stringConverters) {
        Map<ConverterType, StringConverter<String, String>> converters =
                stringConverters.stream().collect(toMap(StringConverter::getName, identity()));
    ...
    public FileTableGeneratorsProvider fileTableServicesProvider(List<FileTableGenerator> generators) {
        Map<FileType, FileTableGenerator> converters =
                generators.stream().collect(toMap(FileTableGenerator::getType, identity()));
    ...
    public BaseTypeValidatorsProvider baseTypeValidatorsProvider(List<BaseTypeValidator> typeValidators) {
        Map<BaseType, BaseTypeValidator> validators =
                typeValidators.stream().collect(toMap(BaseTypeValidator::getBaseType, identity()));
        return new BaseTypeValidatorsProvider(validators);
    }

    @Bean
    public ConfigMapperServicesProvider configMapperServicesProvider(
            List<ConfigMapperService<? extends Config>> configMapperServices) {
        Map<String, ConfigMapperService<? extends Config>> mappers =
                configMapperServices.stream().collect(toMap(ConfigMapperService::getType, identity()));
        return new ConfigMapperServicesProvider(mappers);
    }

    @Bean
    public BaseTypeGeneratorServicesProvider baseTypeGeneratorServicesProvider(
            List<BaseTypeGeneratorService> baseTypeGeneratorServices) {
        Map<BaseType, BaseTypeGeneratorService> generatorServices =
                baseTypeGeneratorServices.stream().collect(toMap(BaseTypeGeneratorService::getType, identity()));
        return new BaseTypeGeneratorServicesProvider(generatorServices);
    }

    @Bean
    public CustomTypeGeneratorServicesProvider customTypeGeneratorServicesProvider(
            List<CustomTypeGeneratorService> customTypeGeneratorServices) {
        Map<CustomType, CustomTypeGeneratorService> generatorServices =
                customTypeGeneratorServices.stream().collect(toMap(CustomTypeGeneratorService::getType, identity()));
    ...
    public ConverterConvertersProvider converterServicesProvider(List<ConverterService> converterServices) {
        Map<BaseType, ConverterService> converterServicesMap =
                converterServices.stream().collect(toMap(ConverterService::getType, identity()));
    ...
    public GenerationHandlersProvider generationHandlersProvider(List<GenerationHandler> generationHandlers) {
        Map<GenerationStrategy, GenerationHandler> handlers =
                generationHandlers.stream().collect(toMap(GenerationHandler::getStrategy, identity()));
    ...
    public CellConfigurersProvider cellFactoriesProvider(List<CellConfigurer> cellFactories) {
        Map<Class<?>, CellConfigurer> factories =
                cellFactories.stream().collect(toMap(CellConfigurer::getType, identity()));
        return new CellConfigurersProvider(factories);
    }

    @Bean
    public FakerGeneratorServicesProvider fakerGeneratorServicesProvider(
            List<FakerGeneratorService> fakerGeneratorServices) {

...
public class ProvidersConfig {
    ...
    public TypeConvertersProvider typeConvertersProvider(List<TypeConverter<?>> typeConverters) {
        Map<BaseType, TypeConverter<?>> converters =
                typeConverters.stream().collect(toMap(TypeConverter::getByType, identity()));
        return new TypeConvertersProvider(converters);
    }

    @Bean
    public DetectorsProvider detectorsProvider(List<Detector<?>> detectors) {
        Map<BaseType, Detector<?>> detectorsList = detectors.stream().collect(toMap(Detector::getByType, identity()));
